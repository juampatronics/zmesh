...
public:
    inline T offset() const { return d2; }
...
public:
    explicit quadratic(const T& a = T(), const T& b = T(), const T& c = T(),
                       const T& d = T())
    {
        init(a, b, c, d);
    }
    ...
    explicit quadratic(const zi::vl::mat<T, 4>& o) { init(o); }
...
#    undef ZI_QUADRATIC_COMPOUND_OPERATOR
    ...
    T evaluate(const T& x, const T& y, const T& z) const
    {
        return (x * (x * a2 + (y * ab + z * ac + ad) * 2.0) +
                y * (y * b2 + (z * bc + bd) * 2.0) + z * (z * c2 + cd * 2.0) +
                d2);
    }
    ...
    {
        return zi::vl::mat<T, 4>(a2, ab, ac, ad, ab, b2, bc, bd, ac, bc, c2, cd,
                                 ad, bd, cd, d2);
    }
    ...
    {
        zi::vl::mat<T, 3> ainv(a2, ab, ac, ab, b2, bc, ac, bc, c2);
    ...
    bool optimize(zi::vl::vec<T, 3>& v, const zi::vl::vec<T, 3>& v1,
...
inline quadratic<typename ::zi::vl::detail::promote<T, Y>::type>
...
template <class T>
inline quadratic<T> operator+(const quadratic<T>& x, const quadratic<T>& y)
{
    quadratic<T> res(x);
    res += y;
    return res;
...
template <class T>
inline quadratic<T> operator+(const quadratic<T>& x, const T& y)
{
    quadratic<T> res(x);
    res += y;
    return res;
...
template <class T>
inline quadratic<T> operator+(const T& y, const quadratic<T>& x)
{
    quadratic<T> res(x);
    res += y;
    return res;
