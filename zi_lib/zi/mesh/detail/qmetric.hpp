...
public:
    inline T offset() const { return offset_; }

    const zi::vl::vec<T, N>& vector() const { return vector_; }

    const zi::vl::mat<T, N>& tensor() const { return tensor_; }
...
public:
    ...
    explicit qmetric(const vl::vec<T, N>& p1, const vl::vec<T, N>& p2,
                     const vl::vec<T, N>& p3)
        : tensor_()
        , vector_()
        , offset_()
    ...
    explicit qmetric(const vl::mat<X, N>& t, const vl::vec<Y, N>& v, const T& o)
        : tensor_(t)
        , vector_(v)
        , offset_(o)
    {
    ...
    explicit qmetric(const vl::mat<T, N>& t, const vl::vec<T, N>& v, const T& o)
        : tensor_(t)
        , vector_(v)
        , offset_(o)
    {
...
#    undef ZI_QMETRIC_COMPOUND_OPERATOR
    ...
    */
    ...
    {
        return dot(v, tensor_ * v) + dot(v, vector_) * static_cast<T>(2) +
               offset_;
    }

    inline T operator()(const vl::vec<T, N>& v) const { return evaluate(v); }
    ...
    /*    inline bool optimize( zi::vl::vec< T, 3 >& v,
          ...
          T q = -( dot( vector(), d ) * 2 + dot( av2, d ) + dot( v2, ad ) ) *
       invdenom;
...
template <class T, std::size_t N>
inline typename zi::vl::detail::enable_if<is_floating_point<T>,
                                          qmetric<T, N>>::type
...
inline qmetric<typename zi::vl::detail::promote<T, O>::type, N>
...
template <class T, std::size_t N>
inline qmetric<T, N> operator+(const qmetric<T, N>& x, const qmetric<T, N>& y)
{
    qmetric<T, N> res(x);
    res += y;
    return res;
...
template <class T, std::size_t N>
inline qmetric<T, N> operator+(const qmetric<T, N>& x)
{
    return x;
}
