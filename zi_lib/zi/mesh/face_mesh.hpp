...
struct floating_point_compare_predicate
{
    ...
    {
        const int64_t l =
            static_cast<int64_t>(static_cast<long double>(lhs) * 10000000);
        const int64_t r =
            static_cast<int64_t>(static_cast<long double>(rhs) * 10000000);
        return l < r;
    }
};
...
{
    bool operator()(const zi::vl::vec<T, N>& lhs,
                    const zi::vl::vec<T, N>& rhs) const
    {
...
public:
    std::vector<point_t>& points() { return points_; }

    const std::vector<point_t>& points() const { return points_; }

    std::vector<point_t>& normals() { return normals_; }

    const std::vector<point_t>& normals() const { return normals_; }

    std::vector<face_t>& faces() { return faces_; }

    const std::vector<face_t>& faces() const { return faces_; }
    ...
    void add(const std::vector<point_t>& p, const std::vector<point_t>& n,
             const std::vector<face_t>& f, const T& x = T(0), const T& y = T(0),
             const T& z = T(0))
    {
        ...
        for (std::size_t i = 0; i < p.size(); ++i)
        {
            ...
            if (it == map_.end())
            {
                map_.insert(std::make_pair(
                    p[i] + off, static_cast<uint32_t>(points_.size())));
        ...
        // std::cout << "Matching overlap: " << matches << " out of " <<
        // p.size() << '\n';
    }

    void add(const point_t* p, const point_t* n, std::size_t psize,
             const face_t* f, std::size_t fsize, const T& x = T(0),
             const T& y = T(0), const T& z = T(0))
    {
        std::size_t matches = 0;
    ...
    void add(const face_mesh& fm, const T& x = T(0), const T& y = T(0),
             const T& z = T(0))
    {
    ...
    std::size_t mem_size() const
    {
        return points_.capacity() * sizeof(point_t) +
               ...
               map_.size() * (sizeof(typename map_t::value_type) +
                              2 * sizeof(std::ptrdiff_t));
    }

    template <class W>
    std::size_t fill_simplifier(::zi::mesh::simplifier<W>& ret,
                                ...
                                const point_t& scale = point_t(1, 1, 1))
    {
        ...
        FOR_EACH(it, faces_) { ret.add_face(it->at(0), it->at(1), it->at(2)); }
