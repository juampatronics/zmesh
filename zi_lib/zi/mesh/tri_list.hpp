...
public:
    typedef typename if_<
        ...
        typename if_<(S > 5), uint32_t,
                     typename if_<(S > 3), uint16_t, uint8_t>::type>::type>::
        type packed_type;

    typedef typename if_<(S > 16), uint32_t,
                         typename if_<(S > 8), uint16_t, uint8_t>::type>::type
        coordinate_type;
...
public:
    ...
    public:
        ...
        inline coordinate(const coordinate_type x, const coordinate_type y,
        ...
        inline void set(const packed_type packed) { packed_ = packed; }

        inline void set(const coordinate_type x, const coordinate_type y,
                        const coordinate_type z)
        {
            packed_ = x;
            packed_ <<= width;
        ...
        {
            return static_cast<coordinate_type>((packed_ >> double_width) &
                                                mask);
        }
        ...
        inline operator packed_type() const { return packed_; }
    };
    ...
    public:
        ...
        inline triangle(const coordinate& c1, const coordinate& c2,
                        const coordinate& c3)
            : c1_(c1)
            , c2_(c2)
            , c3_(c3)
        ...
        inline triangle(const packed_type c1, const packed_type c2,
                        const packed_type c3)
            : c1_(c1)
            , c2_(c2)
            , c3_(c3)
        {
...
public:
    ...
    inline triangle& operator[](std::size_t i) { return v_[i]; }

    inline const triangle& operator[](std::size_t i) const { return v_[i]; }

    inline void clear() { v_.clear(); }

    inline void insert(const packed_type c1, const packed_type c2,
                       const packed_type c3)
    {
