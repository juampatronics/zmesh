...
private:
    ...
    static inline uint32_t face_of(uint32_t e) { return e & face_mask; }
    ...
    inline uint32_t is_valid(uint32_t w) const { return (meta_[w] & 1); }
    ...
    inline void unvalidate(uint32_t w) { meta_[w] &= 0xfe; }
    ...
    inline uint32_t degree(uint32_t f) const { return (meta_[f] >> 4); }
    ...
    typedef binary_heap<
        heap_entry,
        zi::heap::hashed_index<zi::heap::member_variable<heap_entry, uint32_t,
                                                         &heap_entry::face_>>,

        zi::heap::value<zi::heap::member_variable<heap_entry, uint32_t,
                                                  &heap_entry::value_>,
                        std::less<uint32_t>>>
        heap_type;
    ...
    inline void extract_strip(uint32_t f, std::vector<uint32_t>& vertices,
                              ...
                              std::vector<uint32_t>& strip_lengths)
    {
        ...
        static const uint32_t circle_edge[4] = {0, 3, 1, 2};

        static const uint32_t expected_idx[2][4] = {{0, 3, 1, 2}, {0, 2, 3, 1}};
        ...
        while (degree(curr_face) != 1)
        {
            ...
            ++length;
            vertices.push_back(
                face_data_[curr_face >> 2].v[(entr_edge & 3) - 1]);
