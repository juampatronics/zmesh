...
inline T left_householder_iteration(mat<T, N>& A, mat<T, N>& U,
                                    const std::size_t i)
{
    static vec<T, N> d;
...
template <class T, std::size_t N>
inline T right_householder_iteration(mat<T, N>& A, mat<T, N>& V,
                                     const std::size_t i)
{
    static vec<T, N> d;
...
inline typename detail::enable_if_c<
    (M + 1 == N),
    typename detail::enable_if<is_floating_point<T>, bool>::type>::type
bidiagonalize(mat<T, N>& A, mat<T, N>& U, mat<T, N>& V, vec<T, N>& diagonal,
              vec<T, M>& super_diagonal)
{
    U = mat<T, N>::eye;
    V = mat<T, N>::eye;
