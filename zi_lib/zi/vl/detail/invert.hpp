...
inline typename detail::enable_if<is_integral<T>, T>::type invert(mat<T, N>& m)
{
    return 0;
}
...
invert(mat<T, 3>& m)
{
    ...
    T d = m.elem(0) * res.elem(0) + m.elem(1) * res.elem(3) +
          m.elem(2) * res.elem(6);
...
invert(mat<T, 4>& m)
{
    ...
    T t1[6] = {m.elem(2) * m.elem(7) - m.elem(6) * m.elem(3),
    ...
    T t2[6] = {m.elem(0) * m.elem(5) - m.elem(4) * m.elem(1),
    ...
    T d = m.elem(0) * res.elem(0) + m.elem(4) * res.elem(1) +
...
inline typename detail::enable_if_c<
    (N > 4), typename detail::enable_if<is_floating_point<T>, T>::type>::type
invert(mat<T, N>& m)
{
...
template <class T, std::size_t N>
inline mat<typename detail::promote<T>::type, N> inv(const mat<T, N>& rhs,
                                                     bool&            ok)
{
