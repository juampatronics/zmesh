...
struct eye_init_tag
{
    explicit eye_init_tag() {}
};
struct trans_init_tag
{
    explicit trans_init_tag() {}
};
struct minor_init_tag
{
    explicit minor_init_tag() {}
};
...
public:
    ...
    explicit mat(const T& rcp = T()) { std::fill_n(begin(), size(), rcp); }
    ...
    explicit mat(const vec<T, N>& diag, const detail::eye_init_tag&)
    {
        std::fill_n(begin(), size(), 0);
    ...
    explicit mat(const T& p00, const T& p01, const T& p10, const T& p11)
    {
    ...
    explicit mat(const T& p00, const T& p01, const T& p02, const T& p10,
                 const T& p11, const T& p12, const T& p20, const T& p21,
                 const T& p22)
    {
    ...
    explicit mat(const mat<T, N + 1>& rcp, size_type r, size_type c,
...
#    undef ZI_VL_INDEX_ACCESSOR_BODY
    ...
    const T* c_array() const { return d; }
    ...
    iterator end() { return d + num_elements; }

    const_iterator end() const { return d + num_elements; }

    reverse_iterator rbegin() { return d + num_elements - 1; }

    reverse_const_iterator rbegin() const { return d + num_elements - 1; }

    reverse_iterator rend() { return d - 1; }

    reverse_const_iterator rend() const { return d - 1; }

    reference front() { return d[0]; }

    const_reference front() const { return d[0]; }

    reference back() { return d[num_elements - 1]; }

    const_reference back() const { return d[num_elements - 1]; }
    ...
    void fill(const T& val) { std::fill_n(begin(), num_elements, val); }

    void assign(const T& val) { std::fill_n(begin(), num_elements, val); }
...
public:
    static size_type size() { return num_elements; }
    ...
    vec<T, N> get_major_diagonal() const { return diagonal(); }

    vec<T, N> get_semimajor_diagonal() const
    {
        vec<T, N> res;
    ...
    /*
        ...
        bool equals( const mat< O, N >& rhs,
                     ...
                     std::numeric_limits< typename detail::promote< T, O >::type
    >::epsilon() ) const
        {
    ...
    */
    ...
    void swap_columns(size_type src_col, size_type target_col,
                      size_type first_row = 0, size_type last_row = N - 1)
    {
    ...
    void swap_rows(size_type src_row, size_type target_row,
                   size_type first_col = 0, size_type last_col = N - 1)
    {
...
inline bool operator==(const mat<T, N>& lhs, const mat<O, N>& rhs)
{
...
#    define ZI_VL_INLINE_BINARY_OPERATOR_SCALAR_TYPE(op)                       \
                                                                               \
        template <class T, std::size_t N>                                      \
        inline typename mat<T, N>::type operator op(const T&         lhs,      \
                                                    const mat<T, N>& rhs)      \
        {                                                                      \
        ...
        inline typename detail::enable_if<                                     \
            is_scalar<T>, mat<typename detail::promote<O, T>::type, N>>::type  \
...
#    define ZI_VL_INLINE_BINARY_OPERATOR_VEC_TYPE(op)                          \
                                                                               \
        template <class T, class O, std::size_t N>                             \
        inline vec<typename detail::promote<T, O>::type, N> operator op(       \
            const vec<T, N>& lhs, const mat<O, N>& rhs)                        \
        {                                                                      \
            ...
            {                                                                  \
                ...
                {                                                              \
                    res.at(r) += static_cast<PT>(lhs.at(c)) * rhs.at(r, c);    \
                }                                                              \
            }                                                                  \
                                                                               \
            return res;                                                        \
                                                                               ...
                                                                               \
        template <class T, std::size_t N>                                      \
        inline vec<T, N> operator op(const vec<T, N>& lhs,                     \
                                     const mat<T, N>& rhs)                     \
        {                                                                      \
            vec<T, N> res;                                                     \
                                                                               \
...
#    define ZI_VL_INLINE_BINARY_OPERATOR_TYPE_VEC(op)                          \
        ...
        template <class T, class O, std::size_t N>                             \
        inline vec<typename detail::promote<T, O>::type, N> operator op(       \
            const mat<T, N>& lhs, const vec<O, N>& rhs)                        \
        {                                                                      \
            typedef typename detail::promote<T, O>::type PT;                   \
            vec<PT, N>                                   res;                  \
                                                                               \
            ...
            {                                                                  \
                ...
                for (std::size_t c = 0; c < N; ++c)                            \
                {                                                              \
                    res.at(r) += static_cast<PT>(rhs.at(c)) * lhs.at(r, c);    \
                }                                                              \
            }                                                                  \
                                                                               \
            return res;                                                        \
                                                                               ...
                                                                               \
        template <class T, std::size_t N>                                      \
        inline vec<T, N> operator op(const mat<T, N>& lhs,                     \
                                     const vec<T, N>& rhs)                     \
        {                                                                      \
            vec<T, N> res;                                                     \
                                                                               \
...
#    define ZI_VL_INLINE_BINARY_OPERATOR_TYPE_SCALAR(op)                       \
        ...
        inline typename detail::enable_if<                                     \
            is_scalar<O>, mat<typename detail::promote<O, T>::type, N>>::type  \
        ...
        {                                                                      \
            ...
            return res;                                                        \
                                                                               ...
                                                                               \
        template <class T, std::size_t N>                                      \
        inline mat<T, N> operator op(const mat<T, N>& lhs, const T& rhs)       \
        {                                                                      \
            mat<T, N> res(lhs);                                                \
            res       op## = rhs;                                              \
            return res;                                                        \
...
#    define ZI_VL_INLINE_BINARY_OPERATOR_TYPE_TYPE(op)                         \
        template <class T, class O, std::size_t N>                             \
        inline mat<typename detail::promote<O, T>::type, N> operator op(       \
            const mat<T, N>& lhs, const mat<O, N>& rhs)                        \
        {                                                                      \
            mat<typename detail::promote<O, T>::type, N> res(lhs);             \
            res                                          op## = rhs;           \
            return res;                                                        \
                                                                               ...
                                                                               \
        template <class T, std::size_t N>                                      \
        inline typename mat<T, N>::type operator op(const mat<T, N>& lhs,      \
                                                    const mat<T, N>& rhs)      \
        {                                                                      \
            mat<T, N> res(lhs);                                                \
            res       op## = rhs;                                              \
...
template <class T, class O, std::size_t N>
inline mat<typename detail::promote<T, O>::type, N>
...
template <class T>
inline mat<T, 1> operator*(const mat<T, 1>& lhs, const mat<T, 1>& rhs)
{
    return mat<T, 1>(lhs.elem(0) * rhs.elem(0));
}

template <class T, class O, std::size_t N>
inline vec<T, N>& operator*=(vec<T, N>& v, const mat<O, N>& m)
{
    v = v * m;
    return v;
}
...
template <class T, std::size_t N>
inline mat<T, N> operator+(const mat<T, N>& rhs)
{
    return rhs;
}
...
typename detail::enable_if_c<(N > 2),
                             ::std::basic_ostream<CharT, Traits>&>::type
operator<<(::std::basic_ostream<CharT, Traits>& os, const mat<T, N>& m)
{
    std::size_t i;
    os << '/';
    for (i = 0; i < N; ++i)
...
operator<<(::std::basic_ostream<CharT, Traits>& os, const mat<T, 1>& m)
{
    return os << "< " << m.elem(0) << " >";
}
...
template <class T, class CharT, class Traits>
::std::basic_ostream<CharT, Traits>&
operator<<(::std::basic_ostream<CharT, Traits>& os, const mat<T, 2>& m)
{
