...
inline mat<T, N> trans(const mat<T, N>& rhs)
{
    return mat<T, N>(rhs, detail::trans_init_tag());
}

template <class T, std::size_t N>
inline void transpose(mat<T, N>& rhs)
{
...
template <class T, std::size_t N>
inline mat<T, N - 1> getminor(const mat<T, N>& m, std::size_t r, std::size_t c)
{
    return mat<T, N - 1>(m, r, c, detail::minor_init_tag());
}

template <class T, std::size_t N>
inline mat<T, N - 1> get_minor(const mat<T, N>& m, std::size_t r, std::size_t c)
{
    return mat<T, N - 1>(m, r, c, detail::minor_init_tag());
}

template <class T>
inline typename detail::promote<T>::type trace(const mat<T, 1>& rhs)
{
    return static_cast<typename detail::promote<T>::type>(rhs.elem(0));
}

template <class T, std::size_t N>
inline typename detail::promote<T>::type trace(const mat<T, N>& rhs)
{
...
template <class T>
inline typename detail::promote<T>::type det(const mat<T, 1>& rhs)
{
    return static_cast<typename detail::promote<T>::type>(rhs.elem(0));
}

template <class T>
inline typename detail::promote<T>::type det(const mat<T, 2>& rhs)
{
    return static_cast<typename detail::promote<T>::type>(rhs.elem(0)) *
               rhs.elem(3) -
           static_cast<typename detail::promote<T>::type>(rhs.elem(2)) *
               rhs.elem(1);
}

template <class T>
inline typename detail::promote<T>::type det(const mat<T, 3>& rhs)
{
    typedef typename detail::promote<T>::type promoted_type;
    return static_cast<promoted_type>(rhs.elem(0)) * rhs.elem(4) * rhs.elem(8) -
...
template <class T>
inline typename detail::promote<T>::type det(const mat<T, 4>& rhs)
{
    typedef typename detail::promote<T>::type promoted_type;
...
template <class T, std::size_t N>
inline typename detail::enable_if_c<(N > 4),
                                    typename detail::promote<T>::type>::type
...
template <class T>
inline mat<typename detail::promote<T>::type, 1> adj(const mat<T, 1>& rhs)
{
    return mat<typename detail::promote<T>::type, 1>(rhs.elem(0));
}

template <class T>
inline mat<typename detail::promote<T>::type, 2> adj(const mat<T, 2>& rhs)
{
...
template <class T, std::size_t N>
inline typename detail::enable_if_c<
    (N > 2), mat<typename detail::promote<T>::type, N>>::type
...
template <class T, std::size_t N>
inline mat<typename detail::promote<T>::type, N> square(const mat<T, N>& m)
{
...
template <class T, std::size_t N>
inline mat<typename detail::promote<T>::type, N> pow(const mat<T, N>& m, int p)
{
    if (p < 0)
    {
        return inv(pow(m, -p));
...
pow(const mat<T, N>& m, typename detail::promote<T>::type p)
{
    ...
    // promoted_type max_val = static_cast< promoted_type >( x.absmax() ) * N *
    // 2;
    ...
    promoted_type vala = two_nfact / (four_to_n * nfact * nfact *
                                      (static_cast<promoted_type>(1) - two_n));
    ...
    while (1)
    {
        ...
        if (std::fabs(bottom) <=
                std::numeric_limits<promoted_type>::epsilon() ||
            std::isnan(bottom))
        {
        ...
        // promoted_type top = static_cast< promoted_type >( neg ? -1 : 1 ) *
        // two_nfact; promoted_type invbottom = static_cast< promoted_type >( 1
        // ) / bottom;
...
template <class T, std::size_t N>
inline mat<typename detail::promote<T>::type, N> sqrt(const mat<T, N>& m)
{
    typedef typename detail::promote<T>::type promoted_type;
...
template <class T, std::size_t N>
inline mat<T, N> make_diag(const vec<T, N>& diag)
{
...
template <class T, std::size_t N>
inline mat<T, N> make_diagonal(const vec<T, N>& diag)
{
    static detail::eye_init_tag tag;
    return mat<T, N>(diag, detail::eye_init_tag());
}
...
template <class T, std::size_t N>
inline typename detail::enable_if_c<
    N == 4, mat<typename detail::promote<T>::type, 4>>::type
...
#    define ZI_VL_OUTER_PRODUCT_IMPL(name)                                     \
        ...
        inline mat<typename detail::promote<T, O>::type, N> name(              \
        ...
        {                                                                      \
            ...
            {                                                                  \
                ...
                {                                                              \
                    res.at(i, j) =                                             \
                        static_cast<promoted_type>(v1.at(i)) * v2.at(j);       \
                }                                                              \
            }                                                                  \
            return res;                                                        \
        }                                                                      \
                                                                               \
        template <class T, std::size_t N>                                      \
        inline mat<T, N> name(const vec<T, N>& v1, const vec<T, N>& v2)        \
        {                                                                      \
            mat<T, N> res;                                                     \
                                                                               \
            for (std::size_t i = 0; i < N; ++i)                                \
