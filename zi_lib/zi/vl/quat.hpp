...
struct quat_identity_tag
{
    explicit quat_identity_tag() {}
};
...
public:
    ...
    explicit quat(const T& q0 = T(), const T& q1 = T(), const T& q2 = T(),
                  const T& q3 = T())
    {
        d[0] = q0;
        d[1] = q1;
    ...
    {
        mat<T, 3> mcopy(m.at(0, 0), m.at(0, 1), m.at(0, 2), m.at(1, 0),
                        m.at(1, 1), m.at(1, 2), m.at(2, 0), m.at(2, 1),
                        m.at(2, 2));
...
#    define ZI_VL_INDEX_ACCESSOR_MEMBER(accessor)                              \
        ...
        reference accessor(size_type i) { ZI_VL_INDEX_ACCESSOR_BODY(i); }

#    define ZI_VL_NAMED_INDEX_ACCESSOR_MEMBER(name, idx)                       \
        const_reference name() const { ZI_VL_INDEX_ACCESSOR_BODY(idx); }       \
        reference       name() { ZI_VL_INDEX_ACCESSOR_BODY(idx); }
...
#    undef ZI_VL_NAMED_INDEX_ACCESSOR_MEMBER
    ...
    quat<T> unreal() const { return quat<T>(d[0], d[1], d[2], 0); }

    vec<T, 3> unreal_vec() const { return vec<T, 3>(d[0], d[1], d[2]); }
    ...
    iterator end() { return d + 4; }

    const_iterator end() const { return d + 4; }

    reverse_iterator rbegin() { return d + 4 - 1; }

    reverse_const_iterator rbegin() const { return d + 4 - 1; }
    ...
    reference back() { return d[4 - 1]; }

    const_reference back() const { return d[4 - 1]; }
    ...
    void fill(const T& val) { d[0] = d[1] = d[2] = d[3] = val; }

    void assign(const T& val) { d[0] = d[1] = d[2] = d[3] = val; }
        ...
        else
        {
            ...
            else
            {
                throw ::std::runtime_error(
                    "quat<>: no max on the matrix diagonal");
            }
        }
    }
...
template <class T>
const quat<T> quat<T>::quater_i(1, 0, 0, 0);
template <class T>
const quat<T> quat<T>::quater_j(0, 1, 0, 0);
template <class T>
const quat<T> quat<T>::quater_k(0, 0, 1, 0);
template <class T>
const quat<T> quat<T>::quat_i(1, 0, 0, 0);
template <class T>
const quat<T> quat<T>::quat_j(0, 1, 0, 0);
template <class T>
const quat<T> quat<T>::quat_k(0, 0, 1, 0);
template <class T>
const quat<T> quat<T>::identity(0, 0, 0, 1);
template <class T>
const quat<T> quat<T>::unit_i(1, 0, 0, 0);
template <class T>
const quat<T> quat<T>::unit_j(0, 1, 0, 0);
template <class T>
const quat<T> quat<T>::unit_k(0, 0, 1, 0);
template <class T>
const quat<T> quat<T>::unit_w(0, 0, 0, 1);
template <class T>
const quat<T> quat<T>::unit(0, 0, 0, 1);
...
inline bool operator==(const quat<T>& lhs, const quat<O>& rhs)
{
...
template <class T, class O>
inline bool operator==(const quat<T>& lhs, const vec<O, 3>& rhs)
{
    return lhs.at(3) == 0 && std::equal(rhs.begin(), rhs.end(), lhs.begin());
}

template <class T>
inline bool operator==(const quat<T>& lhs, const vec<T, 3>& rhs)
{
    return lhs.at(3) == 0 && std::equal(rhs.begin(), rhs.end(), lhs.begin());
}

template <class T, class O>
inline bool operator==(const vec<T, 3>& lhs, const quat<O>& rhs)
{
    return rhs.at(3) == 0 && std::equal(lhs.begin(), lhs.end(), rhs.begin());
}

template <class T>
inline bool operator==(const vec<T, 3>& lhs, const quat<T>& rhs)
{
    return rhs.at(3) == 0 && std::equal(lhs.begin(), lhs.end(), rhs.begin());
}

template <class T>
inline bool operator==(const T& lhs, const quat<T>& rhs)
{
    return rhs.at(3) == lhs && rhs.at(0) == 0 && rhs.at(1) == 0 &&
           rhs.at(2) == 0;
}

template <class T>
inline bool operator==(const quat<T>& lhs, const T& rhs)
{
    return lhs.at(3) == rhs && lhs.at(0) == 0 && lhs.at(1) == 0 &&
           lhs.at(2) == 0;
}
...
#    define ZI_VL_INLINE_BINARY_OPERATOR_SCALAR_TYPE(op)                       \
                                                                               \
        template <class T>                                                     \
        inline typename quat<T>::type operator op(const T&       lhs,          \
                                                  const quat<T>& rhs)          \
        {                                                                      \
            quat<T> res(rhs);                                                  \
            res     op## = lhs;                                                \
        ...
        inline typename detail::enable_if<                                     \
            is_scalar<T>, quat<typename detail::promote<O, T>::type>>::type    \
        operator op(const T& lhs, const quat<O>& rhs)                          \
        {                                                                      \
            quat<typename detail::promote<O, T>::type> res(rhs);               \
            res                                        op## = lhs;             \
            return res;                                                        \
        }
...
#    define ZI_VL_INLINE_BINARY_OPERATOR_TYPE_SCALAR(op)                       \
        ...
        inline typename detail::enable_if<                                     \
            is_scalar<O>, quat<typename detail::promote<O, T>::type>>::type    \
        ...
        {                                                                      \
            ...
            return res;                                                        \
                                                                               ...
                                                                               \
        template <class T>                                                     \
        inline quat<T> operator op(const quat<T>& lhs, const T& rhs)           \
        {                                                                      \
            quat<T> res(lhs);                                                  \
            res     op## = rhs;                                                \
            return res;                                                        \
...
#    define ZI_VL_INLINE_BINARY_OPERATOR_TYPE_TYPE(op)                         \
        template <class T, class O>                                            \
        inline quat<typename detail::promote<O, T>::type> operator op(         \
            const quat<T>& lhs, const quat<O>& rhs)                            \
        {                                                                      \
            quat<typename detail::promote<O, T>::type> res(lhs);               \
            res                                        op## = rhs;             \
            return res;                                                        \
                                                                               ...
                                                                               \
        template <class T>                                                     \
        inline typename quat<T>::type operator op(const quat<T>& lhs,          \
                                                  const quat<T>& rhs)          \
        {                                                                      \
            quat<T> res(lhs);                                                  \
            res     op## = rhs;                                                \
...
#    define ZI_VL_INLINE_BINARY_OPERATOR_TYPE_VEC3(op)                         \
        template <class T, class O>                                            \
        inline quat<typename detail::promote<O, T>::type> operator op(         \
            const quat<T>& lhs, const vec<O, 3>& rhs)                          \
        {                                                                      \
            quat<typename detail::promote<O, T>::type> res(lhs);               \
            res                                        op## = rhs;             \
            return res;                                                        \
...
operator<<(::std::basic_ostream<CharT, Traits>& os, const quat<T>& q)
{
