...
public:
    ...
    explicit vec(const T& rcp = T()) { std::fill_n(begin(), N, rcp); }
...
#    undef ZI_VL_NAMED_INDEX_ACCESSOR_MEMBER
    ...
    reverse_iterator rbegin() { return d + N - 1; }

    reverse_const_iterator rbegin() const { return d + N - 1; }
    ...
    reference back() { return d[N - 1]; }

    const_reference back() const { return d[N - 1]; }
    ...
    void fill(const T& val) { std::fill_n(begin(), N, val); }

    void assign(const T& val) { std::fill_n(begin(), N, val); }
...
inline bool operator==(const vec<T, N>& lhs, const vec<O, N>& rhs)
{
...
#    define ZI_VL_INLINE_BINARY_OPERATOR_SCALAR_TYPE(op)                       \
                                                                               \
        template <class T, std::size_t N>                                      \
        inline typename vec<T, N>::type operator op(const T&         lhs,      \
                                                    const vec<T, N>& rhs)      \
        {                                                                      \
            vec<T, N> res(lhs);                                                \
            res       op## = rhs;                                              \
        ...
        inline typename detail::enable_if<                                     \
            is_scalar<T>, vec<typename detail::promote<O, T>::type, N>>::type  \
...
#    define ZI_VL_INLINE_BINARY_OPERATOR_TYPE_SCALAR(op)                       \
        ...
        inline typename detail::enable_if<                                     \
            is_scalar<O>, vec<typename detail::promote<O, T>::type, N>>::type  \
        ...
        {                                                                      \
            ...
            return res;                                                        \
                                                                               ...
                                                                               \
        template <class T, std::size_t N>                                      \
        inline vec<T, N> operator op(const vec<T, N>& lhs, const T& rhs)       \
        {                                                                      \
            vec<T, N> res(lhs);                                                \
            res       op## = rhs;                                              \
            return res;                                                        \
...
#    define ZI_VL_INLINE_BINARY_OPERATOR_TYPE_TYPE(op)                         \
        template <class T, class O, std::size_t N>                             \
        inline vec<typename detail::promote<O, T>::type, N> operator op(       \
            const vec<T, N>& lhs, const vec<O, N>& rhs)                        \
        {                                                                      \
            vec<typename detail::promote<O, T>::type, N> res(lhs);             \
            res                                          op## = rhs;           \
            return res;                                                        \
                                                                               ...
                                                                               \
        template <class T, std::size_t N>                                      \
        inline typename vec<T, N>::type operator op(const vec<T, N>& lhs,      \
                                                    const vec<T, N>& rhs)      \
        {                                                                      \
            vec<T, N> res(lhs);                                                \
            res       op## = rhs;                                              \
...
operator<<(::std::basic_ostream<CharT, Traits>& os, const vec<T, N>& v)
{
    os << "[ " << v[0];
