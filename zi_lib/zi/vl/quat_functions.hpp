...
{
    return q.at(0) * q.at(0) + q.at(1) * q.at(1) + q.at(2) * q.at(2) +
           q.at(3) * q.at(3);
}
...
template <class T>
inline quat<T> inv(const quat<T>& q, bool& ok)
{
    quat<T> tmp(q);
    ok = invert(tmp);
    return tmp;
...
inline typename detail::promote<T, O>::type dot(const quat<T>& lhs,
                                                const quat<O>& rhs)
{
    typedef typename detail::promote<T>::type promoted_type;

    return static_cast<promoted_type>(lhs.at(0)) * rhs.at(0) +
...
inline T dot(const quat<T>& lhs, const quat<T>& rhs)
{
    return lhs.at(0) * rhs.at(0) + lhs.at(1) * rhs.at(1) +
           lhs.at(2) * rhs.at(2) + lhs.at(3) * rhs.at(3);
}

template <class T, class O>
inline vec<typename detail::promote<T, O>::type, 3> cross(const quat<T>& lhs,
                                                          const quat<O>& rhs)
{
    typedef typename detail::promote<T>::type promoted_type;
...
template <class T>
inline vec<T, 3> cross(const quat<T>& lhs, const quat<T>& rhs)
{
    return vec<T, 3>(lhs.at(1) * rhs.at(2) - lhs.at(2) * rhs.at(1),
                     lhs.at(2) * rhs.at(0) - lhs.at(0) * rhs.at(2),
                     lhs.at(0) * rhs.at(1) - lhs.at(1) * rhs.at(0));
}

template <class T>
inline quat<T> cross(const quat<T>& q0, const quat<T>& q1, const quat<T>& q2)
{
    quat<T> res;

    res[0] = q0[1] * (q1[2] * q2[3] - q1[3] * q2[2]) +
             q1[1] * (q2[2] * q0[3] - q2[3] * q0[2]) +
             q2[1] * (q0[2] * q1[3] - q0[3] * q1[2]);

    res[1] = q0[0] * (q1[2] * q2[3] - q1[3] * q2[2]) +
             q1[0] * (q2[2] * q0[3] - q2[3] * q0[2]) +
             q2[0] * (q0[2] * q1[3] - q0[3] * q1[2]);

    res[2] = q0[0] * (q1[1] * q2[3] - q1[3] * q2[1]) +
             q1[0] * (q2[1] * q0[3] - q2[3] * q0[1]) +
             q2[0] * (q0[1] * q1[3] - q0[3] * q1[1]);

    res[3] = q0[0] * (q1[1] * q2[2] - q1[2] * q2[1]) +
...
inline quat<typename detail::promote<T, O, W>::type>
...
template <class T>
inline quat<T> normal(const quat<T>& qa, const quat<T>& qb, const quat<T>& qc,
                      const quat<T>& qd)
{
    quat<T> quat_t(qb - qa);
    quat<T> quat_u(qc - qa);
...
inline quat<typename detail::promote<X, Y, Z, W>::type>
normal(const quat<X>& qa, const quat<Y>& qb, const quat<Z>& qc,
       const quat<W>& qd)
{
    typedef typename detail::promote<X, Y, Z, W>::type promoted_type;
    quat<promoted_type>                                quat_t(qb);
    quat_t -= qa;
    quat<promoted_type> quat_u(qc);
    quat_u -= qa;
    quat<promoted_type> quat_v(qd);
    quat_v -= qa;
...
template <class T, class O>
inline quat<T>& operator/=(quat<T>& lhs, const quat<O>& rhs)
{
...
template <class T, class O>
inline quat<typename detail::promote<T, O>::type>
...
template <class T>
inline quat<T> slerp(const T& a, const quat<T>& p, const quat<T>& q)
{
    quat<T> np = norm(p);
    quat<T> nq = norm(q);
