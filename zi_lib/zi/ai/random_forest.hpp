...
public:
    ...
    explicit random_forest(uint32_t n, const std::vector<T>& patterns,
                            ...
                            const Splitter*              splitter)
    {
        ...
        for (uint32_t j = 0; j < total; ++j)
        {
            ...
            {
                uint32_t rnd =
                    static_cast<uint32_t>(std::rand() % positives->size());
                bag_positives.push_back(positives->operator[](rnd));
            }
            else
            {
                uint32_t rnd =
                    static_cast<uint32_t>(std::rand() % negatives->size());
 ...
 /*
         ...
         // balance the data
         while ( bag_negatives.size() > bag_positives.size() &&
 bag_negatives.size() > 0 )
         {
             std::swap( bag_negatives[ std::rand() % bag_negatives.size() ],
 bag_negatives.back() ); bag_negatives.pop_back();
         }

         while ( bag_negatives.size() < bag_positives.size() &&
 bag_positives.size() > 0 )
         {
             std::swap( bag_positives[ std::rand() % bag_positives.size() ],
 bag_positives.back() ); bag_positives.pop_back();
         }
 */
    ...
    void create(uint32_t n, const std::vector<T>& patterns,
                const std::vector<uint32_t>& positives,
                const std::vector<uint32_t>& negatives,
                const Splitter&              splitter = Splitter())
    {
        ...
        for (uint32_t i = 0; i < n; ++i)
        {
            tm.push_back(zi::run_fn(
                zi::bind(&this_random_forest_type::create_single_tree, this,
    ...
    double eval(const T& pattern) const
    {
        if (trees_.size())
        {
            ...
            FOR_EACH(it, trees_) { res += it->eval(pattern); }
    ...
    double operator()(const T& pattern) const { return eval(pattern); }
    ...
    std::string to_string() const
    {
        ...
        if (trees_.size())
        {
            FOR_EACH(it, trees_) { iss << it->to_string() << "\n"; }
        }
        return iss.str();
    }
    ...
    void dump() const
    {
        if (trees_.size())
        {
            FOR_EACH(it, trees_) { std::cout << it->to_string() << "\n"; }
        }
    }
