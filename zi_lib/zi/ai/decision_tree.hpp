...
private:
    ...
    public:
        ...
        inline double operator()(const T& t) const { return eval(t); }
    ...
    public:
        ...
        double eval(const T&) const { return probability_; }
        ...
        void dump_to_file(std::ofstream& ofs) const { ofs << probability_; }
    };
    ...
    public:
        interior_node(const std::vector<T>&  patterns,
                      ...
                      const Splitter& splitter, double weight_positive = 1)
            : split_fn_(splitter.get_split_fn(patterns, positives, negatives,
                                              weight_positive))
            , left_(0)
            , right_(0)
            , depth_(0)
        {
            ...
            {
                left_  = new leaf_node(total_positive, total_negative,
                                       weight_positive);
                depth_ = 1;
            }
            else
            {
                left_ =
                    new interior_node(patterns, left_positives, left_negatives,
                                      splitter, weight_positive);
                right_ = new interior_node(patterns, right_positives,
                                           right_negatives, splitter,
                                           weight_positive);
        ...
        void dump_to_file(std::ofstream& ofs) const
        {
            ...
            {
                ofs << "c " << split_fn_.get_index() << " "
                    << split_fn_.get_threshold() << " ";
                left_->dump_to_file(ofs);
                ofs << " ";
        ...
        //         iss << "( " <<
        //         boost::lexical_cast<std::string>(split_fn_.get_index()) << "
        //         " <<
...
public:
                ...
                double                       weight_positive = 1)
    {
        ...
        else
        {
            ...
            root_node_ = shared_ptr<interior_node>(
                new interior_node(patterns, positives_copy, negatives_copy,
                                  splitter, weight_positive));
        }
    }
    ...
    inline double operator()(const T& pattern) const { return eval(pattern); }

    inline uint32_t depth() const { return root_node_->depth(); }

    std::string to_string() const { return root_node_->to_string(); }
