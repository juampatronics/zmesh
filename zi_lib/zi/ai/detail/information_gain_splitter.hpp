...
public:
    ...
    public:
        explicit split_fn(uint32_t index, T threshold, bool all_same = false)
            : index_(index)
        ...
        bool is_dummy() const { return all_same_; }
    };
                          ...
                          double weight_positive = 1) const
    {
        ...
        double original_entropy =
            detail::entropy(positives.size(), negatives.size());
        ...
        for (uint32_t i = 0; i < Sub; ++i)
        {
            ...
            for (uint32_t p_idx = 0, n_idx = 0; p_idx < curr_positives.size() ||
                                                n_idx < curr_negatives.size();)
            {
                T new_threshold =
                    0; // std::min( ( n_idx < curr_negatives.size() ) ?
                //    curr_negatives[ n_idx ] : curr_positives[ p_idx ],
                //                          ( p_idx < curr_positives.size() ) ?
                //                          curr_positives[ p_idx ] :
                //                          curr_negatives[ n_idx ] );
                ...
                if (new_threshold != curr_threshold)
                {
                    ...
                    double curr_entropy =
                        ...
                        (detail::entropy(weight_positive *
                                             (positives_total - p_idx),
    ...
    information_gain<T, Sub> next() const { return *this; }
};
