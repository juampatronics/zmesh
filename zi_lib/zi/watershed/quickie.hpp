...
public:
    quickie_impl(const T* conn, const std::size_t x_dim,
                 const std::size_t y_dim, const std::size_t z_dim,
                 const std::size_t size_thold, const T low_thold,
                 const T hi_thold, const T abs_low_thold, R* result)
        : connections_(conn)
        , x_dim_(x_dim)
        , y_dim_(y_dim)
        , z_dim_(z_dim)
    ...
    void doit()
    {
        ...
        for (std::size_t d = 0; d < 3; ++d)
        {
            ...
            {
                tm.push_back(zi::run_fn(zi::bind(
                    &quickie_impl::add_hi_threshold_edges_thread, this, d, z)));
            }
        }
