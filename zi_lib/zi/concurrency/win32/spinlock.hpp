...
public:
    spinlock() { win32::InitializeCriticalSection(&cs_); }

    ~spinlock() { win32::DeleteCriticalSection(&cs_); }
    ...
    inline void lock() const { win32::EnterCriticalSection(&cs_); }

    inline void unlock() const { win32::LeaveCriticalSection(&cs_); }
