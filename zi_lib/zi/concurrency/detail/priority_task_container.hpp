...
public:
    explicit priority_task_container()
        : queue_()
        , counter_(0)
    {
    }
    ...
    std::size_t empty() const { return queue_.empty(); }

    void clear() { queue_.clear(); }

    shared_ptr<runnable> front() { return queue_.begin()->second; }

    void pop_front() { queue_.erase(queue_.begin()); }

    template <class Priority>
    void push_front(shared_ptr<runnable> task)
    {
        queue_.insert(
            std::make_pair(-(Priority::value << 48) + (++counter_), task));
    }
