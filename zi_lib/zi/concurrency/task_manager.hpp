...
public:
    ...
    std::size_t empty() { return manager_->empty(); }

    std::size_t idle() { return manager_->empty(); }

    std::size_t size() { return manager_->size(); }
    ...
    bool start() { return manager_->start(); }

    void stop(bool and_join = false) { return manager_->stop(and_join); }

    void join() { manager_->join(); }
    ...
    void
    push_front(shared_ptr<Runnable> task, std::size_t count = 1,
    ...
    void push_front(const Function& task,
                    typename meta::enable_if<typename is_convertible<
                        Function, function<void()>>::type>::type* = 0)
    {
    ...
    template <class Runnable>
    void
    push_back(shared_ptr<Runnable> task, std::size_t count = 1,
              typename meta::enable_if<
                  typename is_base_of<runnable, Runnable>::type>::type* = 0)
    {
        for (; count > 0; --count)
    ...
    template <class Function>
    void push_back(const Function& task, std::size_t count = 1,
                   typename meta::enable_if<typename is_convertible<
                       Function, function<void()>>::type>::type* = 0)
    {
    ...
    void add_workers(std::size_t count) { manager_->add_workers(count); }
...
typedef concurrency_::task_manager_tpl<
    concurrency_::detail::simple_task_container>
    simple;
typedef concurrency_::task_manager_tpl<
    concurrency_::detail::simple_task_container>
    deque;
typedef concurrency_::task_manager_tpl<
    concurrency_::detail::priority_task_container>
    prioritized;
