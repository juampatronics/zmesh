...
public:
    ...
    {
    public:
        read_guard() { class_rwmutex<Class>::instance().acquire_read(); }

        ~read_guard() { class_rwmutex<Class>::instance().release_read(); }
    };

    class write_guard : zi::non_copyable
    {
    public:
        write_guard() { class_rwmutex<Class>::instance().acquire_write(); }

        ~write_guard() { class_rwmutex<Class>::instance().release_write(); }
    };
};
