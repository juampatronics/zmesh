...
public:
    ...
    explicit monitor(const mutex& m)
        : m_(m)
        , cv_()
        , waiters_(0)
    {
...
private:
    ...
    void notify_one() const { cv_.notify_one(); }
...
public:
    ...
    public:
        ...
        void wait() const { m_.wait(); }

        bool timed_wait(int64_t ttl) const { return m_.timed_wait(ttl); }

        void notify_one() const { m_.notify_one(); }

        void notify_all() const { m_.notify_all(); }
        ...
        void wait_for(F f, const A1& a1, const A2& a2, const A3& a3,
                      const A4& a4) const
        {
            while (!f(a1, a2, a3, a4))
            {
