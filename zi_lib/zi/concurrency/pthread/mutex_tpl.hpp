...
public:
    mutex_tpl() { mutex_initializer<PtMutexTag>::initialize(mutex_); }
...
#    ifndef ZI_OS_MACOS
        ...
        if (!errno)
        {
            ...
            std::cerr
                << "mutex_tpl: destructor: pthread_mutex_destroy error: \n\t"
...
#    endif
    ...
    inline void lock() const { ZI_VERIFY_0(pthread_mutex_lock(&mutex_)); }

    inline void unlock() const { ZI_VERIFY_0(pthread_mutex_unlock(&mutex_)); }
