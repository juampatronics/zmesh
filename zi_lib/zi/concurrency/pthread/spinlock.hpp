...
public:
    ...
    ~spinlock() { ZI_VERIFY_0(pthread_spin_destroy(&spin_)); }

    inline bool try_lock() const { return pthread_spin_trylock(&spin_) == 0; }

    inline void lock() const { ZI_VERIFY_0(pthread_spin_lock(&spin_)); }

    inline void unlock() const { ZI_VERIFY_0(pthread_spin_unlock(&spin_)); }
