...
public:
    ...
    ~rwmutex() { ZI_VERIFY_0(pthread_rwlock_destroy(&m_)); }
    ...
    inline void release() const { ZI_VERIFY_0(pthread_rwlock_unlock(&m_)); }
    ...
    public:
        explicit read_guard(const rwmutex& m)
            : m_(m)
        {
            m_.acquire_read();
        }
    ...
    public:
        explicit write_guard(const rwmutex& m)
            : m_(m)
        {
            m_.acquire_write();
        }
