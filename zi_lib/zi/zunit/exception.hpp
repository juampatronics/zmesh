...
public:
    ...
    explicit exception(const Exception& e, int line)
        : type_(UNKNOWN)
        , test_(0)
        , line_(line)
        , check_()
    ...
    virtual const char* arg(int) const { return ""; }

    virtual const char* arg_type(int) const { return ""; }

    virtual const char* arg_value(int) const { return ""; }
    ...
    virtual const char* what() const throw()
    {
        if (message_.empty())
        {
            ...
            case BINARY_CHECK:
                ...
                ss << arg(0) << " = " << arg_value(0) << " < " << arg_type(0)
                   << " >\n\t\t";

                ss << arg(1) << " = " << arg_value(1) << " < " << arg_type(1)
                   << " >\n\t\t";
...
template <class T>
struct choose_exception
{
    typedef std::exception type;
};

template <>
struct choose_exception<std::exception>
{
