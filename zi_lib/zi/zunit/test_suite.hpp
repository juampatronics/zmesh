...
public:
    ...
    int run_all()
    {
        ...
        tout << name_ << " ( " << files_.size() << " files, " << tests_.size()
             << " tests )\n"
             << tos::reset;
        ...
        FOR_EACH(it, tests_)
        {
            tout << "Test: " << std::setw(40) << std::left << it->get().name()
                 << tos::flush;
        ...
        tout << std::setw(46) << std::setfill('-') << ""
             << "+--------+------------------------------------\n"
             << std::setfill(' ') << std::setw(45) << std::left << "Suite total"
             << " |";
...
template <class SuiteTag>
struct suite_name;

template <>
struct suite_name<default_suite_tag>
{
