...
{
private:
    static const double pi_over_2_deg =
        1.5707963267948966192313216916397514L / 180.0;
    ...
    private:
        ...
        camera_translation_impl(const vl::vec3f& center, camera* c, int x,
                                int y)
            : camera_operation_impl(c, x, y)
            , center_(center)
        {
    ...
    void finish_operation_() { operation_.reset(); }

    void apply_rotation_drag(const vl::mat4f& r, int sxi, int syi, int cxi,
                             int cyi)
    {
    ...
    void apply_translation_drag(const vl::vec3f& c, int sx, int sy, int cx,
                                int cy)
    {
        ...
        center_ = c +
                  dr * vl::vec3f(rotation_.at(0, 0), rotation_.at(0, 1),
                                 rotation_.at(0, 2)) +
                  du * vl::vec3f(rotation_.at(1, 0), rotation_.at(1, 1),
                                 rotation_.at(1, 2));
    }
...
public:
    ...
    public:
        ...
        void update(int x, int y) { op_->update(x, y); }
    ...
    void perspective(float fov) { perspective_.at(0) = fov; }

    void center(const vl::vec3f& center) { center_ = center; }

    void center(float x, float y, float z) { center_.set(x, y, z); }

    vl::vec3f center() const { return center_; }

    void distance(float d) { distance_ = d; }

    float distance() const { return distance_; }

    void rotation(const vl::mat4f& rot) { rotation_ = rot; }

    vl::mat4f rotation() const { return rotation_; }
    ...
    {
        glViewport(viewport_.at(0), viewport_.at(1), viewport_.at(2),
                   viewport_.at(3));
    }
