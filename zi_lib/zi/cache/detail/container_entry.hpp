...
struct container_entry
{
    ...
    template <class CacheType>
    static group_t to_group_(CacheType* value_ptr)
    {
    ...
    explicit container_entry(int64_t token, int64_t timestamp, CacheType& cb,
                             const Key& key)
        : token_(token)
        , group_(to_group_(cb))
    ...
    template <class Key>
    explicit container_entry(int64_t token, int64_t timestamp, const Key& key,
                             bool (&cb)(Key const&))
        : token_(token)
        , group_(to_group_(cb))
        , timestamp_(timestamp)
    ...
    container_entry(int64_t token, int64_t timestamp, group_t group,
                    callback_t& cb)
        : token_(token)
        , group_(group)
        , timestamp_(timestamp)
    ...
    bool try_flush() const { return flush_callback_(); }

    struct timestamp_updater
    {
        void operator()(container_entry& e) { e.timestamp_ = now::usec(); }
    };
};
