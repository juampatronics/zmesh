...
struct storage_entry
{
    ...
    storage_entry(int64_t token, int64_t group, const callback_t& callback,
                  int64_t size = 0)
        : token_(token)
        , group_(group)
        , timestamp_(now::usec())
    ...
    void flush() const { flush_callback_(); }

    struct timestamp_updater
    {
        void operator()(storage_entry& e) { e.timestamp_ = now::usec(); }
    };
    ...
    public:
        size_updater(int64_t size)
            : size_(size)
        {
        }

        void operator()(storage_entry& e) { e.size_ = size_; }
    };
};
