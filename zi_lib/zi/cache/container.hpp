...
inline int64_t cache_container<Tag>::insert(CacheType& cb_obj, const Key& key)
{
    guard g(mutex_);
...
template <class Tag>
template <class Key>
inline int64_t cache_container<Tag>::insert(bool (&cb_fn)(Key const&),
                                            const Key& key)
{
    guard g(mutex_);
...
template <class Tag>
template <class Key>
inline int64_t cache_container<Tag>::insert(int64_t group, const Key& key,
                                            function<bool(void)>& cb_fn)
{
    guard g(mutex_);
...
inline std::size_t cache_container<Tag>::count_of(CacheType& t) const
{
    guard g(mutex_);
...
template <class Tag>
template <class CacheType>
inline std::size_t cache_container<Tag>::clear_group(CacheType& t)
{
    guard g(mutex_);
...
template <class Tag>
inline bool cache_container<Tag>::touch(int64_t token)
{
    guard      g(mutex_);
...
template <class Tag>
inline bool cache_container<Tag>::remove(int64_t token)
{
...
template <class Tag>
inline int64_t cache_container<Tag>::age_of(int64_t token)
{
    guard      g(mutex_);
    iterator_t it = token_index_.find(token);
...
template <class Tag>
inline int64_t cache_container<Tag>::oldest_age()
{
    guard g(mutex_);
...
template <class Tag>
inline int64_t cache_container<Tag>::oldest_token()
{
    guard g(mutex_);
...
template <class Tag>
inline std::size_t cache_container<Tag>::size() const
{
    return index_.size();
}
...
template <class Tag>
inline void cache_container<Tag>::lock()
{
    mutex_.lock();
}

template <class Tag>
inline void cache_container<Tag>::unlock()
{
    mutex_.unlock();
}
