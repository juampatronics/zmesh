...
#    define ZI_MATH_DETAIL_FFT8_DEFINE_FN(name, one_or_four, four_or_one,      \
                                          three_or_six, six_or_three, inv_c1,  \
                                          inv_c2, inv_c3)                      \
                                                                               \
        template <class T>                                                     \
        ...
        {                                                                      \
            ...
            f[0]               = t7 + t8;                                      \
            f[four_or_one]     = t7 - t8;                                      \
            std::complex<T> m4 = constants<T>::half_root_two() * (t3 - t6);    \
                                                                               \
            std::complex<T> m7 = std::complex<T>(0, inv_c1) * (t3 + t6);       \
                                                                               \
...
ZI_MATH_DETAIL_FFT8_DEFINE_FN(dit_forward, 1, 4, 3, 6,
                              -constants<T>::half_root_two(), t5 - t4,
                              f[3] - f[2])

ZI_MATH_DETAIL_FFT8_DEFINE_FN(dit_inverse, 1, 4, 3, 6,
                              constants<T>::half_root_two(), t4 - t5,
                              f[2] - f[3])
