...
inline atomic_word compare_swap(volatile atomic_word* ptr, atomic_word val,
                                atomic_word cmp)
{
    return ::__sync_val_compare_and_swap(const_cast<atomic_word*>(ptr), cmp,
                                         val);
}
...
inline void increment(volatile atomic_word* ptr) { (void)add_swap(ptr, 1); }
...
inline void write(volatile atomic_word* ptr, atomic_word val) { *ptr = val; }

inline atomic_word read(volatile atomic_word* ptr) { return *ptr; }
...
inline atomic_word test_increment_swap(volatile atomic_word* ptr)
{
    ...
    } while (!::__sync_bool_compare_and_swap(const_cast<atomic_word*>(ptr), ret,
                                             ret + 1));
