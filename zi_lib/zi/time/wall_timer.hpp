...
public:
    ...
    inline void restart() { start_ = lap_start_ = now::nsec(); }
    ...
    template <class T>
    inline T
    lap_elapsed(typename enable_if<is_fundamental<T>::value>::type* = 0)
    {
    ...
    template <class T>
    inline int64_t
    elapsed(typename disable_if<is_fundamental<T>::value>::type* = 0)
    {
    ...
    template <class T>
    inline int64_t
    lap_elapsed(typename disable_if<is_fundamental<T>::value>::type* = 0)
    {
        return convert_(now::nsec() - lap_start_, T::factor);
    }

    inline int64_t lap() { return lap<int64_t>(); }

    inline int64_t elapsed() { return elapsed<int64_t>(); }

    inline int64_t lap_elapsed() { return lap_elapsed<int64_t>(); }
    ...
    public:
        ...
        scoped(const T& cb,
               typename meta::enable_if<typename is_convertible<
                   T, zi::function<void(int64_t)>>::type>::type* = 0)
            : start_(now::msec())
            , f_(cb)
        {
        }

        ~scoped() { f_(now::msec() - start_); }
    };
};
